...
`app.py` is a Streamlit layer over the model — the trade-off curve as something
you operate rather than read.

Drag the **loss-given-default** and **net margin** sliders and every figure on the
page recomputes: the break-even, the profit-maximising cutoff, the approval-rate
curve, the risk bands, the sensitivity grid. That's the argument of the policy
section made tangible — the cutoff belongs to the business, not to the model.
...
The app reads a pre-scored CSV (`data/processed/scored_test.csv`, regenerated by
the last cell of the notebook) instead of loading the model, so it starts
instantly and needs no xgboost or scikit-learn at runtime.

One implementation note worth calling out: the rows are sorted once by score, then
cumulative sums of exposure are precomputed for good and bad accounts separately.
Approving "everyone below cutoff *t*" is then a prefix of that sorted array, so
each candidate cutoff is an **O(1) lookup rather than an O(n) mask** — and changing
LGD or margin only rescales the cumsums. That's what keeps the sliders instant
instead of recomputing 2.4M operations per drag.

A **Power BI** version of the same views lives in [`powerbi/`](powerbi/), built
from the same exported fact tables.
...
**Two requirements files, deliberately.** `requirements.txt` holds only what the
deployed app needs (four packages); `requirements-notebook.txt` adds the full
modelling stack — TensorFlow, XGBoost, LightGBM, SHAP, LIME and the rest. Keeping
them separate is what lets the app deploy to a free tier: installing TensorFlow to
serve a dashboard that never imports it would blow the memory budget.
